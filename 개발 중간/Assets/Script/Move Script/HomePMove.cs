...
public class HomePMove : MonoBehaviour
{
    ...
    void Portal()
    {
        ...
        if (pPos.x >= -293 && pPos.x <= -243 && pPos.y <= -103.0f && Input.GetKeyDown("w"))//1>2층 위로 이동
        {

            player.transform.position = new Vector3(pPos.x, 136.0f, 0f);//좌표
        }

        if (pPos.x >= -293 && pPos.x <= -243 && pPos.y >= 136.0f && Input.GetKeyDown("s"))//2>1층 아래로 이동
        {

            player.transform.position = new Vector3(pPos.x, -103.6f, 0f);//좌표
        }
        if (pPos.x >= -293 && pPos.x <= -243 && pPos.y <= -91.0f && pPos.y >= -104.0f && Input.GetKeyDown("s"))//1>B1층 아래로 이동
        {

            player.transform.position = new Vector3(pPos.x, -302.5f, 0f);//좌표
        }
    ...
    void Wall()
    {
        ...
        if(pPos.y >= -104.0f && pPos.y <= -102.0f) //1층 이동제한
        ...
        if(pPos.y >= -303.0f && pPos.y <= -301.0f) //B1층 이동제한
